...
class CW(Attach):
    def __init__(self, model, c=1e-1, kappa=1e-10, steps=10000, lr=1e-1, binary_search_steps=9):
    ...
    def forward(self, images, labels):
        ...
        lower_bound = torch.zeros(batch_size, dtype=torch.float)
        const = torch.ones(batch_size, dtype=torch.float) * self.c
        upper_bound = torch.ones(batch_size, dtype=torch.float) * 1e9

        outer_success = torch.empty(batch_size, dtype=torch.bool) * torch.tensor([False])
        for outer_step in range(self.binary_search_steps):
            ...
            prev_cost = 1e10
            success = torch.empty(batch_size, dtype=torch.bool) * torch.tensor([False])
        ...
        return best_adv_images, outer_success
